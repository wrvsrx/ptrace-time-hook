...
int parse_args(int argc, char *argv[], int *target_argc, char ***target_argv) {
  ...
  while ((c = getopt_long(argc, argv, "vt:h", long_options, &option_index)) !=
         -1) {
    switch (c) {
    case 'v':
      verbose = 1;
...
int main(int argc, char *argv[]) {
  ...
  if (verbose) {
    printf("ptrace-time-hook: Target program: %s\n", target_argv[0]);
    printf("ptrace-time-hook: Target time: %ld\n", target_time);
  }
  ...
  while (1) {
    ...
    if (regs.orig_rax == SYS_time) {
      if (verbose) {
        printf("ptrace-time-hook: Found time() syscall\n");
      }
      ...
      if (verbose) {
        printf("ptrace-time-hook: Modified time() return to %ld\n",
               target_time);
      }
    }
  }
  ...
  if (verbose) {
    printf("ptrace-time-hook: Process finished\n");
  }
  return 0;
}